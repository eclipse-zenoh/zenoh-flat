...
use zenoh_flat::{zbytes_as_bytes, zbytes_new_clone, zbytes_new_from_slice, zbytes_new_from_vec};
...
fn slice_round_trip() {
    ...
    assert_eq!(zbytes_as_bytes(&payload).as_ref(), data);
}
...
fn clone_shares_payload() {
    ...
    assert_eq!(zbytes_as_bytes(&cloned).as_ref(), data);
    assert_eq!(zbytes_as_bytes(&payload), zbytes_as_bytes(&cloned));
}
